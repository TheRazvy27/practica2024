...
def incarca_date(nume_fisier):
    ...
    if extensie == "csv":
        with open(nume_fisier, mode="r") as my_file:
            for line in my_file:
                date = line.split(',')
                if date[0] == 'Nume':
                    continue
                print (date)
                if verifica_duplicat(date[2]):
                    dict_cursant = {"id": _id, "nume": date[0], "prenume": date[1], "cnp": date[2]}
                    _id += 1
                    lista_cursanti.append(dict_cursant)
...
def modificare(id):
    ...
    for cursant in lista_cursanti:
        if cursant['id'] == id:
            print ('Modificati cursantul cu datele:', cursant['nume'], cursant['prenume'], "CNP:", cursant['cnp'])
            alegere = input("Ce doriti sa modificati la cursant? Nume, Prenume, CNP: ")
            if alegere == "Nume":
                modif = input ("Introduceti numele nou dorit: ")
                cursant['nume'] = modif
                print('Numele a fost actualizat cu succes!')
            if alegere == "CNP":
                modif = input("Introduceti CNP-ul dorit: ")
                if validare_cnp(modif):
                    if verifica_duplicat(modif):
                        cursant['CNP'] = modif
                        print("CNP-ul a fost actualizat cu succes!")
            if alegere == "Prenume": 
                modif = input ("Introduceti prenumele nou dorit: ")
                cursant['prenume'] = modif

def concatenare(fisier1, fisier2):
    extensie1 = fisier1.split(".")[1]
    extensie2 = fisier2.split('.')[1]
    if not extensie1 == extensie2:
        print("Fisierele trebuie sa fie de acelasi tip! ")
        return False
    incarca_date(fisier1)
    incarca_date(fisier2)
    salveaza_date(extensie1)
    print("Fisierele au fost concatenate cu succes!")
...
if __name__ == "__main__":
    while True:
        dict_cursant = dict()
        new_line = input("Introduceti un nou cursant sau introduceti EXIT/SAVE/DELETE/DISPLAY/LOAD/MODIFY/UNITE.")
        if new_line == "EXIT":
        ...
        if new_line == "LOAD":
            ...
            continue
        if new_line == "MODIFY":
            idmodif = input("Introduceti ID-ul pe care doriti sa il modificati: ")
            modificare(int(idmodif))
            continue
        if new_line == "UNITE":
            fisier1 = input("Introduceti numele primului fisier: ")
            fisier2 = input("Introduceti numele fisierului secundar: ")
            concatenare(fisier1, fisier2)
            continue
